...
import type { CrawlPage, CrawlOptions, CrawlResult } from "./types";
...
async function loadRobotsDisallow(origin: string): Promise<string[]> {
  ...
  } catch { /* robots fetch failure = allow */ }
  ...
  while (queue.length > 0 && pages.length < maxPages) {
    ...
    const results = await Promise.all(
      batch.map(async (item) => {
        ...
        try {
          const res = await fetchWithRetry(item.url, { timeoutMs: 12_000, maxBytes: 500_000 }, 1);
          ...
          return { kind: "error" as const, url: item.url, message: (e as Error).message };
        }
      })
    );
