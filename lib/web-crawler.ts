// PromptMika v3.6.0 web tools — MCP registration layer.
...
import { htmlToText, htmlToMarkdown, extractLinks } from "./web/html";

export function registerWebTools(server: McpServer): void {
  server.tool(
    "web_curl",
    "cURL-like HTTP request. Returns full response: status code, response headers, body, timing, redirect count. Supports any method, custom headers, body, cookies, and follow_redirects toggle. Useful for debugging APIs, checking response headers, inspecting redirect chains. SSRF-guarded, 1 MB cap.",
    {
      url: z.string().min(1).describe("URL to request (http/https only)"),
      method: z.enum(["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"]).default("GET").describe("HTTP method (default GET)"),
      headers: z.record(z.string(), z.string()).optional().describe("Custom request headers (key-value pairs)"),
      body: z.string().optional().describe("Request body for POST/PUT/PATCH"),
      follow_redirects: z.boolean().default(true).describe("Follow redirects (default true). Set false to see 3xx response."),
      timeout_ms: z.number().int().min(1_000).max(30_000).optional().describe("Timeout in ms (default 15000, max 30000)"),
      cookies: z.record(z.string(), z.string()).optional().describe("Cookies to send (key-value pairs)"),
    },
    async ({ url, method, headers, body, follow_redirects, timeout_ms, cookies }) => {
      try {
        ...
        const lines: string[] = [];
        lines.push(`> ${(method ?? "GET").toUpperCase()} ${url}`);
        lines.push(`> Host: ${new URL(url).host}`);
        if (headers) {
          for (const [k, v] of Object.entries(headers)) {
            lines.push(`> ${k}: ${v}`);
          }
        }
        if (cookies) lines.push(`> Cookie: ${Object.entries(cookies).map(([k, v]) => `${k}=${v}`).join("; ")}`);
        lines.push("");
        ...
        lines.push(`// ${res.bytes} bytes received`);
        lines.push("");
        ...
        return { content: [{ type: "text" as const, text: `web_curl error: ${(e as Error).message}` }], isError: true };
      }
    }
  );
  ...
  server.tool(
    "web_fetch",
    "Fetch any URL over HTTP(S): method, custom headers, request body, redirects. Extract modes: raw, text (readable article), markdown (with links), links (all hrefs), json (pretty-printed if parseable). SSRF-guarded, 1 MB size cap, timeout configurable.",
    {
  ...
  server.tool(
    "web_crawl",
    "Crawl a site: breadth-first, depth-limited, same-domain by default, robots.txt respected, configurable delay between requests. Returns title + content per page with timing info.",
    {
      url: z.string().min(1).describe("Seed URL to start crawling from"),
      max_pages: z.number().int().min(1).max(25).default(8).describe("Max pages to fetch (default 8, max 25)"),
      max_depth: z.number().int().min(0).max(5).default(2).describe("Max link depth from seed (default 2, 0 = seed only)"),
      ...
      delay_ms: z.number().int().min(0).max(5000).default(300).describe("Delay between requests in ms (default 300, max 5000)"),
      respect_robots: z.boolean().default(true).describe("Respect robots.txt (default true)"),
    },
    async ({ url, max_pages, max_depth, same_domain, extract, delay_ms, respect_robots }) => {
      try {
        const { pages, skipped, errors } = await crawlSite(url, {
          ...
          delayMs: delay_ms ?? 300,
          respectRobots: respect_robots ?? true,
        });
        if (pages.length === 0) {
        ...
        const sections = pages.map(
          (p, i) => `## [${i + 1}/${pages.length}] ${p.url}\nDepth: ${p.depth} | HTTP ${p.status} | ${p.links} links\n${p.title ? `# ${p.title}\n\n` : ""}${p.content || "(empty)"}`
        );
  ...
  server.tool(
    "browser_scrape",
    "Scrape JS-rendered or Cloudflare-protected pages. Multi-strategy: Jina Reader (free) → Google Cache (free) → Direct fetch. Returns title + markdown.",
    {
      url: z.string().min(1).describe("URL to scrape"),
      strategies: z.array(z.enum(["jina", "google-cache", "direct"])).optional().describe("Strategy order (default: jina, google-cache, direct)"),
    },
    async ({ url, strategies }) => {
      try {
        const { title, text, source } = await scrapeUrl(url, strategies ? { strategies } : {});
