...
async function readBodyLimited(res: Response, maxBytes: number): Promise<{ bytes: number; text: string }> {
  ...
  while (true) {
    ...
    if (done) break;
    if (total + value.length > maxBytes) { await reader.cancel(); break; }
    chunks.push(value);
    total += value.length;
  }
...
function pickHeaders(res: Response): Record<string, string> {
  ...
  res.headers.forEach((v, k) => { out[k] = v.length > 200 ? `${v.slice(0, 200)}...` : v; });
  return out;
}
...
export async function fetchGuarded(rawUrl: string, opts: FetchOptions = {}): Promise<FetchResult> {
  const method = (opts.method ?? "GET").toUpperCase();
  const timeoutMs = Math.min(opts.timeoutMs ?? DEFAULT_TIMEOUT, 30_000);
  const maxBytes = Math.min(opts.maxBytes ?? DEFAULT_MAX, HARD_MAX);
  const followRedirects = (opts as any).followRedirects ?? true;
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    try {
      res = await fetch(url, {
        method,
        headers,
        body: method === "GET" || method === "HEAD" || method === "DELETE" ? undefined : opts.body,
        redirect: "manual",
        signal: controller.signal,
      });
    } catch (e) {
      throw new Error((e as Error).name === "AbortError" ? `Timeout after ${timeoutMs}ms` : `Fetch failed: ${(e as Error).message}`);
    } finally {
      clearTimeout(timer);
    }
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      if (!loc) return { status: res.status, statusText: res.statusText, finalUrl, headers: pickHeaders(res), bytes: 0, text: "" };
      try { url = new URL(loc, url); } catch { throw new Error(`Invalid redirect Location: ${loc}`); }
...
export async function fetchWithRetry(rawUrl: string, opts: FetchOptions = {}, retries = 2): Promise<FetchResult> {
