...
export const WEB_TEMPLATES: WebTemplate[] = [
  ...
  { id: 2, name: "Color Room", folder: "2-color-room-frontend", kind: "frontend", description: "Immersive gallery-like frontend with editorial typography, spatial scenes, and restrained interface chrome.", liveDemo: "https://colorroom.vercel.app/" },
  ...
  { id: 17, name: "Kaleo", folder: "kaleo-style", kind: "style", description: "Warm editorial direction with expressive typography, organic color, and cinematic scroll rhythm." },
...
export function listWebTemplates(kind?: string): WebTemplate[] {
  if (!kind || kind === "all") return WEB_TEMPLATES;
  return WEB_TEMPLATES.filter((template) => template.kind === kind);
}

export function searchWebTemplateLibrary(query: string): Array<WebTemplate & { score: number }> {
  const normalized = query.toLowerCase().trim();
  const words = normalized.split(/\s+/).filter(Boolean);
  return WEB_TEMPLATES.map((template) => {
...
export function resolveWebTemplate(nameOrId: string | number): WebTemplate | undefined {
  const value = String(nameOrId).trim().toLowerCase();
  return WEB_TEMPLATES.find((template) =>
