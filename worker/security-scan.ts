...
const RULES: ScanRule[] = [
  ...
  // SEC-091 removed: the line-based engine cannot detect the ABSENCE of
  // "IMPLICIT NONE" (the actual best practice), and its old pattern flagged
  // files that HAD it — a false positive by construction.
...
function detectLanguage(filePath: string): string {
  ...
  // dotenv family: .env.local, .env.production, .env.development …
  if (name.startsWith(".env") || name === "env.local") return "env";
